# DamperCaseBody 선형 이동 애니메이션 구현 분석

## 0. 참고 레퍼런스
- [`AnimatorAgent.ts:1071-1110`](src/services/AnimatorAgent.ts:1071-1110) - 호출 코드
- [`DamperCaseBodyAnimationService.ts:42-165`](src/services/fridge/DamperCaseBodyAnimationService.ts:42-165) - 구현 코드
- [`assembly-offsets.json:84-112`](public/metadata/assembly-offsets.json:84-112) - 메타데이터 설정

---

## ■■ Description ■■
- **`animateDamperCaseBodyLinearMove` 함수**는 댐퍼 케이스 바디(DamperCaseBody)를 힌지 반대 방향으로 선형 이동시키는 애니메이션을 구현합니다.
- 이 함수는 GSAP 라이브러리를 사용하여 부드러운 이징 효과를 적용하고, 메타데이터 기반의 오프셋 설정을 활용합니다.
...
### 2-3. 노드 검색 및 현재 위치 확인 단계 (Lines 86-96)
...
### 2-5. GSAP 애니메이션 실행 단계 (Lines 124-159)
...
### 5-1. 호출 코드 (AnimatorAgent.ts:1071-1110)
